...
public class PostgresTicketGateway implements TicketDsGateway {

    private final DSLContext create;
    private final RecordMapper<Record, TicketDsModel> ticketMapper = r ->
            new TicketDsModel(
                    r.get(TICKETS.ID),
                    r.get(TICKETS.NAME),
    ...
    public PostgresTicketGateway(DataSource dataSource) {
    ...
    private static TicketUsageLimitsRecord insertUsageLimit(DSLContext dslContext, TicketDsModel ticket) {
    ...
    private static TicketTimeLimitsRecord insertTimeLimit(DSLContext dslContext, TicketDsModel ticket) {
    ...
    @Override
    public List<UsageDsModel> getTicketUsages(UUID id) {
        if (!existsById(id)) {
        ...
        return create.select()
                ...
                .fetch(r -> new UsageDsModel(r.get(METRO_STATIONS.NAME),
                        r.get(TICKET_USAGES.TIMESTAMP).toZonedDateTime(),
                        fromDbUsageType(r)));
    }

    @Override
    public void saveTicketUsage(UUID ticketId, UsageDsModel usage) {
    ...
    private TicketUsageType toDbUsageType(UsageDsModel usage) {
